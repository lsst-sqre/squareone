...
// about the listing UI (covered by OIDCClientsPageClient's own tests).
vi.mock('./OIDCClientsPageClient', () => ({
  default: () => <div>OIDC clients page body</div>,
}));
...
import OIDCClientsPage, { generateMetadata } from './page';
...
function renderPageWithScopes(scopes: string[]) {
  ...
  render(<OIDCClientsPage />);
}

describe('OIDCClientsPage generateMetadata', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  test('sets the page title from config.siteName', async () => {
    ...
    expect(metadata.title).toBe('OIDC clients | Rubin Science Platform');
  });
  ...
  test('uses the configured siteName when it differs', async () => {
    ...
    expect(metadata.title).toBe('OIDC clients | Telescope Ops');
  });
});

describe('OIDCClientsPage scope gate', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  test('renders the page for a user holding the oidcClients scope', () => {
    ...
    expect(screen.getByText('OIDC clients page body')).toBeInTheDocument();
  });
  ...
  test('renders the unauthorized note for a direct visit without the scope', () => {
    ...
    expect(
      screen.queryByText('OIDC clients page body')
