...
describe('NewServiceTokenPage generateMetadata', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  test('sets the page title from config.siteName', async () => {
    ...
    expect(metadata.title).toBe(
      'Create a service token | Rubin Science Platform'
    );
  });
  ...
  test('uses the configured siteName when it differs', async () => {
    ...
    expect(metadata.title).toBe('Create a service token | Telescope Ops');
  });
});
