...
an IVOA document. Every curated RSP service maps to a standard — including
DataLink and GMS, which now link to the IVOA DataLink and GMS docs — so a service
shows no icon only when it is absent from the curated presentation map.
